...
import { Card, Form, Switch, Button, Space, message, Modal, Table, Tabs, Tag, Select } from 'antd';
...
type VpsStatus = { vpsId: number; timezone: string; enableNtp: boolean };
...
export default function TimezoneConfig() {
  ...
  const refreshServerStatus = async () => {
    ...
    try {
      ...
      const results = await Promise.all(
        onlineVps.map((vps) =>
          configModuleApi.getVps('timezone', vps.id)
  ...
  const handleSave = async () => {
    try {
      ...
      setSaving(true);
      await configModuleApi.update('timezone', values);
            ...
            children: (
              <Card loading={loading}>
                <Form
                  form={form}
                  layout="vertical"
                  initialValues={{ timezone: 'UTC', enableNtp: true }}
                >
                  <Form.Item
                    name="timezone"
                    label="时区"
                  ...
                  <Form.Item name="enableNtp" label="网络时间同步">
            ...
            children: (
              <Card>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}>
                  <div style={{ color: '#666' }}>查看服务器当前时区配置状态</div>
                    ...
                    },
                    {
                      ...
                      render: (_: unknown, record: VpsListItem) => (
                        <Tag color={vpsStatusMap[record.id] ? 'blue' : 'warning'}>
                          {vpsStatusMap[record.id]?.timezone || '未知'}
                        </Tag>
                      ),
                    },
                    {
                      ...
                      key: 'enableNtp',
                      render: (_: unknown, record: VpsListItem) => (
                        <Tag color={vpsStatusMap[record.id] ? (vpsStatusMap[record.id]?.enableNtp ? 'green' : 'default') : 'warning'}>
                          {vpsStatusMap[record.id] ? (vpsStatusMap[record.id]?.enableNtp ? '开启' : '关闭') : '未知'}
                        </Tag>
                      ),
                    },
                    {
                      ...
                      render: (_: unknown, record: VpsListItem) => (
                        <Button
                          ...
                          onClick={async () => {
                            try {
                              const res = await configModuleApi.getVps('timezone', record.id);
      ...
      <Modal
        ...
        onOk={async () => {
          ...
          try {
            ...
            const res = await configModuleApi.updateVps('timezone', detailVpsId, values);
            if (res.data?.success) {
              message.success('同步完成');
            }
