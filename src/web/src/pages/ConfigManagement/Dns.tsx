...
type DnsConfig = {
  servers: string; // text representation
};
...
export default function DnsConfigPage() {
  ...
  const { register, handleSubmit, reset } = useForm<DnsConfig>();
  const { register: regDetail, handleSubmit: subDetail, reset: resetDetail } = useForm<DnsConfig>();
  ...
  const fetchConfig = async () => {
    ...
    try {
      ...
      reset({ servers: toText(res.data.content?.servers || []) });
  ...
  const refreshServerStatus = async () => {
    ...
    const results = await Promise.all(
      onlineVps.map(v => 
        configModuleApi.getVps('dns', v.id)
  ...
  const onSave = async (data: DnsConfig) => {
    const servers = parseServers(data.servers);
    const invalid = servers.filter(v => !isValidServer(v));
    if (invalid.length > 0) {
      message.error(`无效DNS: ${invalid.join(', ')}`);
      return;
    }
  ...
  const openDetail = async (id: number) => {
    try {
      const res = await configModuleApi.getVps('dns', id);
      resetDetail({ servers: toText(res.data.content?.servers || []) });
      setDetailVpsId(id);
      setDetailOpen(true);
    } catch (err) {
  ...
  const onDetailSave = async (data: DnsConfig) => {
    ...
    try {
      ...
      setDetailOpen(false);
      setVpsStatusMap(prev => ({ ...prev, [detailVpsId]: { servers, locked: true } }));
    } catch (err) {
      message.error('同步失败');
    } finally {
    ...
    <div className="space-y-6">
            ...
            <CardContent className="space-y-6">
              <form onSubmit={handleSubmit(onSave)} className="space-y-4">
                <div className="space-y-2">
                  <Label>DNS 地址列表</Label>
                  <Textarea {...register('servers')} rows={8} placeholder="8.8.8.8\n1.1.1.1" className="font-mono text-xs" />
                  <p className="text-xs text-slate-500">每行一个地址，仅支持 IP 或域名</p>
                </div>
                  ...
                  {allVps.map(vps => {
                    ...
                    return (
                      <TableRow key={vps.id}>
                        ...
                        <TableCell className="max-w-[200px] truncate" title={status?.servers?.join(', ')}>
                           {status?.servers ? status.servers.join(', ') : <span className="text-slate-400">-</span>}
                        </TableCell>
                        <TableCell>
                           {status ? (status.locked ? <Badge variant="success">已锁定</Badge> : <Badge variant="secondary">默认</Badge>) : <span className="text-slate-400">-</span>}
                        </TableCell>
      ...
      <Dialog open={syncOpen} onOpenChange={setSyncOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader><DialogTitle>同步DNS配置</DialogTitle></DialogHeader>
      ...
      <Dialog open={detailOpen} onOpenChange={setDetailOpen}>
        <DialogContent>
          ...
          <form onSubmit={subDetail(onDetailSave)} className="space-y-4">
             <div className="space-y-2">
                <Label>DNS 地址</Label>
                <Textarea {...regDetail('servers')} rows={8} className="font-mono text-xs" />
             </div>
             <DialogFooter>
